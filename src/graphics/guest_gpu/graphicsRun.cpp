...
private:
	...
	PS5SIM_CLASS_NO_COPY(CommandProcessorGuestAccessScope);
};
...
public:
	...
	~GraphicsRing() { PS5SIM_NOT_IMPLEMENTED; }

	PS5SIM_CLASS_NO_COPY(GraphicsRing);
...
public:
	ComputeRing() = default;
	~ComputeRing() { PS5SIM_NOT_IMPLEMENTED; }

	PS5SIM_CLASS_NO_COPY(ComputeRing);
...
public:
	...
	~Gpu() { PS5SIM_NOT_IMPLEMENTED; }

	PS5SIM_CLASS_NO_COPY(Gpu);
...
void ComputeRing::ThreadRun(void* data) {
	...
	PS5SIM_PROFILER_THREAD("Thread_Compute");
...
void CommandProcessor::Run(uint32_t* data, uint32_t num_dw) {
	PS5SIM_PROFILER_BLOCK("CommandProcessor::Run");
	...
	for (;;) {
		...
		if (GraphicsRunDebugDumpEnabled()) {
			LOGF("CP packet: offset=0x%05" PRIx32 " cmd_id=0x%08" PRIx32 " op=0x%02" PRIx32
			     " len=%" PRIu32 "\n",
			     num_dw - dw, cmd_id, op, PS5SIM_PM4_LEN(cmd_id));
		}

		if ((cmd_id & 1u) != 0 && ShouldSkipPredicatedPackets()) {
			auto packet_len = PS5SIM_PM4_LEN(cmd_id);
			...
			if (skip_log_count.fetch_add(1) < 2048) {
				LOGF("\t predicated skip: op=0x%02" PRIx32 ", r=0x%02" PRIx32 ", len=%" PRIu32
				     ...
				     op, PS5SIM_PM4_R(cmd_id), packet_len, reinterpret_cast<uint64_t>(cmd - 1),
				     cmd_id);
			}
			if (op == Pm4::IT_NOP && PS5SIM_PM4_R(cmd_id) == Pm4::R_RELEASE_MEM && packet_len >= 7) {
