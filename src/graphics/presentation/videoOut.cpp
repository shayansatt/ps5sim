...
public:
	...
	virtual ~FlipQueue() { PS5SIM_NOT_IMPLEMENTED; }
	PS5SIM_CLASS_NO_COPY(FlipQueue);
...
public:
	...
	virtual ~VideoOutContext() { PS5SIM_NOT_IMPLEMENTED; }

	PS5SIM_CLASS_NO_COPY(VideoOutContext);
...
bool FlipQueue::Flip(uint32_t micros) {
	PS5SIM_PROFILER_BLOCK("FlipQueue::Flip");
	...
	// --- شروع کد FPS Counter ---
    // --- شمارنده فریم بدون نیاز به ImGui ---
    ...
    if (std::chrono::duration_cast<std::chrono::seconds>(now - last_time).count() >= 1) {
        ...
        frame_count = 0;
        last_time = now;
        printf("\rFPS: %d      ", fps); // استفاده از \r برای اینکه فقط یک خط آپدیت بشه
        fflush(stdout);
    }
    // ---------------------------------------
    // --- پایان کد FPS Counter ---
...
PS5SIM_SYSV_ABI int VideoOutDeleteFlipEvent(EventQueue::KernelEqueue eq, int handle) {
	PRINT_NAME();
...
PS5SIM_SYSV_ABI int VideoOutAddFlipEvent(EventQueue::KernelEqueue eq, int handle, void* udata) {
	PRINT_NAME();
...
PS5SIM_SYSV_ABI int VideoOutDeleteVblankEvent(EventQueue::KernelEqueue eq, int handle) {
	PRINT_NAME();
...
PS5SIM_SYSV_ABI int VideoOutDeletePreVblankStartEvent(EventQueue::KernelEqueue eq, int handle) {
	PRINT_NAME();
...
PS5SIM_SYSV_ABI int VideoOutGetEventId(const EventQueue::KernelEvent* ev) {
	PRINT_NAME();
...
PS5SIM_SYSV_ABI int VideoOutGetEventData(const EventQueue::KernelEvent* ev, int64_t* data) {
	PRINT_NAME();
...
PS5SIM_SYSV_ABI int VideoOutGetEventCount(const EventQueue::KernelEvent* ev) {
	PRINT_NAME();
