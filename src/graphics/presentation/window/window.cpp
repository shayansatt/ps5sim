...
constexpr uint32_t PS5SIM_SDL_BUTTON_LMASK  = SDL_BUTTON_LMASK;  // NOLINT(hicpp-signed-bitwise)
constexpr uint32_t PS5SIM_SDL_BUTTON_MMASK  = SDL_BUTTON_MMASK;  // NOLINT(hicpp-signed-bitwise)
constexpr uint32_t PS5SIM_SDL_BUTTON_RMASK  = SDL_BUTTON_RMASK;  // NOLINT(hicpp-signed-bitwise)
constexpr uint32_t PS5SIM_SDL_BUTTON_X1MASK = SDL_BUTTON_X1MASK; // NOLINT(hicpp-signed-bitwise)
constexpr uint32_t PS5SIM_SDL_BUTTON_X2MASK = SDL_BUTTON_X2MASK; // NOLINT(hicpp-signed-bitwise)
...
constexpr const char* PS5SIM_SDL_WINDOW_CAPTION = "Game";
constexpr uint32_t    PS5SIM_SDL_WINDOW_FLAGS =
    (static_cast<uint32_t>(SDL_WINDOW_HIDDEN) | static_cast<uint32_t>(SDL_WINDOW_VULKAN));
constexpr int PS5SIM_SDL_WINDOWPOS_CENTERED = SDL_WINDOWPOS_CENTERED; /*NOLINT(hicpp-signed-bitwise)*/
	...
	switch (display.event) {
		...
		case static_cast<Uint8>(DisplayOrientation::DisplayEventOrientation): {
			LOGF("Display %" PRIu32 "[%s] changed orientation to %d - ", display.display,
			     sdl ? "SDL" : "Ps5Sim", static_cast<int>(display.data1));
...
void GameProcessEvent(WindowGame* game, double time_s) {
	...
	switch (event->type) {
		...
		case SDL_MOUSEMOTION: {
			...
			mb.down              = false;
			mb.up                = false;
			mb.left              = ((event->motion.state & PS5SIM_SDL_BUTTON_LMASK) != 0u);
			mb.middle            = ((event->motion.state & PS5SIM_SDL_BUTTON_MMASK) != 0u);
			mb.right             = ((event->motion.state & PS5SIM_SDL_BUTTON_RMASK) != 0u);
			mb.x1                = ((event->motion.state & PS5SIM_SDL_BUTTON_X1MASK) != 0u);
			mb.x2                = ((event->motion.state & PS5SIM_SDL_BUTTON_X2MASK) != 0u);
...
static void WindowCreate(WindowContext* ctx) {
	...
	ctx->window =
	    SDL_CreateWindow(PS5SIM_SDL_WINDOW_CAPTION, PS5SIM_SDL_WINDOWPOS_CENTERED,
	                     PS5SIM_SDL_WINDOWPOS_CENTERED, width, height, PS5SIM_SDL_WINDOW_FLAGS);
...
void WindowRun() {
	...
	PS5SIM_PROFILER_THREAD("Thread_Window");
