...
PS5SIM_SUBSYSTEM_INIT(Graphics) {
...
PS5SIM_SUBSYSTEM_UNEXPECTED_SHUTDOWN(Graphics) {}

PS5SIM_SUBSYSTEM_DESTROY(Graphics) {}
...
struct CommandBuffer {
	using Callback = PS5SIM_SYSV_ABI bool (*)(CommandBuffer*, uint32_t, void*);
	...
	[[nodiscard]] PS5SIM_SYSV_ABI uint32_t GetAvailableSizeDW() const {
	...
	PS5SIM_SYSV_ABI bool ReserveDW(uint32_t num_dw) {
	...
	PS5SIM_SYSV_ABI uint32_t* AllocateDW(uint32_t size_dw) {
...
static void reg_indirect_write_packet(uint32_t* cmd, uint64_t vaddr, uint32_t num_regs,
	...
	cmd[0] = PS5SIM_PM4(5, reg_indirect_native_op(type), reg_indirect_pm4_r(type));
...
static void set_interpolant_register(ShaderRegister* regs, uint32_t index, uint32_t value) {
	// The native implementation writes a descriptor offset (0x10000000 + index). Ps5Sim stores
...
int PS5SIM_SYSV_ABI GraphicsGetDataPacketPayloadRange(MemoryRange* range, uint32_t* cmd, int type) {
	...
	const auto len    = PS5SIM_PM4_LEN(cmd_id);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsCbNop(CommandBuffer* buf, uint32_t size_in_dwords) {
	...
	cmd[0] = PS5SIM_PM4(size_in_dwords, Pm4::IT_NOP, Pm4::R_ZERO);
	if (size_in_dwords > 1) {
...
uint32_t* PS5SIM_SYSV_ABI GraphicsCbDispatch(CommandBuffer* buf, uint32_t thread_group_x,
	...
	cmd[0] = PS5SIM_PM4(5, Pm4::IT_DISPATCH_DIRECT, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsCbBranch(CommandBuffer* buf, uint8_t mode, uint8_t compare_function,
	...
	cmd[0]  = PS5SIM_PM4(14, Pm4::IT_INDIRECT_BUFFER, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsCbSetShRegisterRangeDirect(CommandBuffer* buf, uint32_t offset,
	...
	cmd[0] = PS5SIM_PM4(num_values + 2, Pm4::IT_SET_SH_REG, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsCbSetShRegistersDirect(CommandBuffer*                 buf,
	...
	for (uint32_t i = 1; i <= num_regs; i++) {
		...
		if (flush) {
			...
			cmd[0] = PS5SIM_PM4(run_count + 2u, Pm4::IT_SET_SH_REG, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsCbReleaseMem(CommandBuffer* buf, uint8_t action, uint16_t gcr_cntl,
	...
	cmd[0] = PS5SIM_PM4(8, Pm4::IT_NOP, Pm4::R_RELEASE_MEM);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsAcbResetQueue(CommandBuffer* buf, uint32_t op) {
	...
	cmd[0] = PS5SIM_PM4(2, Pm4::IT_NOP, Pm4::R_DISPATCH_RESET);
	cmd[1] = 0;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbResetQueue(CommandBuffer* buf, uint32_t op, uint32_t state) {
	...
	cmd[0] = PS5SIM_PM4(2, Pm4::IT_CLEAR_STATE, 0u);
	cmd[1] = state & 0xfu;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbWaitUntilSafeForRendering(CommandBuffer* buf,
	...
	cmd[0] = PS5SIM_PM4(7, Pm4::IT_NOP, Pm4::R_WAIT_FLIP_DONE);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetWorkloadsActive(CommandBuffer* buf, uint32_t stream_id,
	...
	cmd[0] = PS5SIM_PM4(WORKLOAD_ACTIVE_PACKET_SIZE_DW, Pm4::IT_NOP, Pm4::R_ZERO);
	std::memset(cmd + 1, 0,
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetWorkloadComplete(CommandBuffer* buf, uint32_t stream_id,
	...
	cmd[0] = PS5SIM_PM4(WORKLOAD_COMPLETE_PACKET_SIZE_DW, Pm4::IT_NOP, Pm4::R_ZERO);
	std::memset(cmd + 1, 0,
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetShRegisterDirect(CommandBuffer* buf, ShaderRegister reg) {
	...
	cmd[0] = PS5SIM_PM4(3, Pm4::IT_SET_SH_REG, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetCxRegisterDirect(CommandBuffer* buf, ShaderRegister reg) {
	...
	cmd[0] = PS5SIM_PM4(3, Pm4::IT_SET_CONTEXT_REG, 0u);
	cmd[1] = reg.offset & 0xffffu;
	cmd[2] = reg.value;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetUcRegisterDirect(CommandBuffer* buf, ShaderRegister reg) {
	...
	cmd[0] = PS5SIM_PM4(3, Pm4::IT_SET_UCONFIG_REG, 0u);
	cmd[1] = reg.offset & 0xffffu;
	cmd[2] = reg.value;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetIndexSize(CommandBuffer* buf, uint8_t index_size,
	...
	cmd[0] = PS5SIM_PM4(3, Pm4::IT_SET_UCONFIG_REG_INDEX, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetIndexBuffer(CommandBuffer* buf, uint64_t index_addr) {
	...
	cmd[0] = PS5SIM_PM4(3, Pm4::IT_INDEX_BASE, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetIndexCount(CommandBuffer* buf, uint32_t index_count) {
	...
	cmd[0] = PS5SIM_PM4(2, Pm4::IT_INDEX_BUFFER_SIZE, 0u);
	cmd[1] = index_count;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetNumInstances(CommandBuffer* buf, uint32_t num_instances) {
	...
	cmd[0] = PS5SIM_PM4(2, Pm4::IT_NUM_INSTANCES, 0u);
	cmd[1] = num_instances;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbDrawIndex(CommandBuffer* buf, uint32_t index_count,
	...
	cmd[0] = PS5SIM_PM4(6, Pm4::IT_DRAW_INDEX_2, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbDrawIndexMultiInstanced(CommandBuffer* buf, uint32_t index_count,
	...
	cmd[0] = PS5SIM_PM4(9, Pm4::IT_DISPATCH_DRAW_PREAMBLE, 0u);
	cmd[1] = index_count;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbDrawIndexAuto(CommandBuffer* buf, uint32_t index_count,
	...
	cmd[0] = PS5SIM_PM4(3, Pm4::IT_DRAW_INDEX_AUTO, 0u);
	cmd[1] = index_count;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbDrawIndexOffset(CommandBuffer* buf, uint32_t index_offset,
	...
	cmd[0] = PS5SIM_PM4(5, Pm4::IT_DRAW_INDEX_OFFSET_2, 0u);
	cmd[1] = (index_count == 0 ? 1u : index_count);
	cmd[2] = index_offset;
	cmd[3] = index_count;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetBaseIndirectArgs(CommandBuffer* buf, uint32_t shader_type,
	...
	cmd[0] = PS5SIM_PM4(4, Pm4::IT_SET_BASE, 0u) | (shader_type << 1u);
	cmd[1] = 1u;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbDrawIndexIndirect(CommandBuffer* buf,
	...
	cmd[0] = PS5SIM_PM4(5, Pm4::IT_DRAW_INDEX_INDIRECT, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbDrawIndirect(CommandBuffer* buf, uint32_t data_offset_in_bytes,
	...
	cmd[0] = PS5SIM_PM4(5, Pm4::IT_DRAW_INDIRECT, 0u);
	cmd[1] = data_offset_in_bytes;
	cmd[2] = static_cast<uint32_t>(patch_offsets);
	cmd[3] = static_cast<uint32_t>(patch_offsets >> 32u);
	cmd[4] = decode_indirect_draw_initiator(modifier);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbDrawIndexIndirectMulti(
	...
	cmd[0] = PS5SIM_PM4(10, Pm4::IT_DRAW_INDEX_INDIRECT_MULTI, 0u);
	cmd[1] = data_offset_in_bytes;
	cmd[2] = static_cast<uint32_t>(patch_offsets);
	cmd[3] = static_cast<uint32_t>(patch_offsets >> 32u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbDispatchIndirect(CommandBuffer* buf,
	...
	cmd[0] = PS5SIM_PM4(3, Pm4::IT_DISPATCH_INDIRECT, 0u);
	cmd[1] = data_offset_in_bytes;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbEventWrite(CommandBuffer* buf, uint8_t event_type,
	...
	cmd[0] = PS5SIM_PM4(addressed_event ? 4u : 2u, Pm4::IT_EVENT_WRITE, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbAcquireMem(CommandBuffer* buf, uint8_t engine, uint32_t cb_db_op,
	...
	cmd[0] = PS5SIM_PM4(8, Pm4::IT_NOP, Pm4::R_ACQUIRE_MEM);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbCondExec(CommandBuffer* buf, const volatile uint32_t* address,
	...
	cmd[0] = PS5SIM_PM4(5, Pm4::IT_COND_EXEC, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsAcbDispatchIndirect(CommandBuffer*       buf,
	...
	cmd[0] = PS5SIM_PM4(4, Pm4::IT_DISPATCH_INDIRECT, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbStallCommandBufferParser(CommandBuffer* buf) {
	...
	cmd[0] = PS5SIM_PM4(2, Pm4::IT_PFP_SYNC_ME, 0u);
	cmd[1] = 0;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbCopyData(CommandBuffer* buf, uint8_t dst,
	...
	cmd[0] = PS5SIM_PM4(6, Pm4::IT_COPY_DATA, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbDmaData(CommandBuffer* buf, uint8_t engine, uint8_t dst,
	...
	cmd[0] = PS5SIM_PM4(7, Pm4::IT_DMA_DATA, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbJump(CommandBuffer* buf, uint8_t mode, uint8_t cache_policy,
	...
	cmd[0] = PS5SIM_PM4(4, Pm4::IT_INDIRECT_BUFFER, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbRewind(CommandBuffer* buf, uint32_t initial_state) {
	...
	cmd[0] = PS5SIM_PM4(2, Pm4::IT_REWIND, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetPredication(CommandBuffer* buf, uint8_t condition, uint8_t op,
	...
	cmd[0] = PS5SIM_PM4(4, Pm4::IT_SET_PREDICATION, 0u);
...
int PS5SIM_SYSV_ABI GraphicsSetPacketPredication(uint32_t* packet, uint32_t predication) {
	...
	LOGF("\t packet0     = 0x%08" PRIx32 ", op = 0x%02" PRIx32 ", r = 0x%02" PRIx32
	     ", len = %" PRIu32 "\n",
	     packet[0], (packet[0] >> 8u) & 0xffu, PS5SIM_PM4_R(packet[0]),
...
int PS5SIM_SYSV_ABI GraphicsSetRangePredication(uint32_t* start, const volatile uint32_t* end,
	...
	while (packet_va < end_va) {
		...
		auto size = PS5SIM_PM4_LEN(cmd_id);
		if ((cmd_id & 0x3fffff00u) == 0x3fff1000u) {
			size = 1;
		}
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbWriteData(CommandBuffer* buf, uint8_t dst, uint8_t cache_policy,
	...
	cmd[0] = PS5SIM_PM4(4 + num_dwords, Pm4::IT_WRITE_DATA, 0u);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbGetLodStats(CommandBuffer* buf, uint8_t cache_policy,
	...
	cmd[0] = PS5SIM_PM4(5, Pm4::IT_GET_LOD_STATS, 0u);
...
int PS5SIM_SYSV_ABI GraphicsWaitRegMemPatchAddress(uint32_t* cmd, const volatile void* address) {
	...
	if (op == Pm4::IT_NOP && PS5SIM_PM4_R(cmd[0]) == Pm4::R_WAIT_MEM_32) {
		cmd[1] = static_cast<uint32_t>(vaddr) & ~0x3u;
		cmd[2] = static_cast<uint32_t>(vaddr >> 32u) & 0x3ffffu;
	} else if (op == Pm4::IT_NOP && PS5SIM_PM4_R(cmd[0]) == Pm4::R_WAIT_MEM_64) {
...
int PS5SIM_SYSV_ABI GraphicsQueueEndOfPipeActionPatchAddress(uint32_t*             cmd,
	...
	if ((op == Pm4::IT_NOP && PS5SIM_PM4_R(cmd[0]) == Pm4::R_RELEASE_MEM) ||
...
int PS5SIM_SYSV_ABI GraphicsQueueEndOfPipeActionPatchData(uint32_t* cmd, uint32_t context_id,
	...
	if ((op == Pm4::IT_NOP && PS5SIM_PM4_R(cmd[0]) == Pm4::R_RELEASE_MEM) ||
		...
		if (op == Pm4::IT_NOP && PS5SIM_PM4_R(cmd[0]) == Pm4::R_RELEASE_MEM && context_id > 1 &&
		    data_sel == 1) {
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbWaitRegMem(CommandBuffer* buf, uint8_t size,
	...
	if (wait32) {
		cmd[0] = PS5SIM_PM4(7, Pm4::IT_NOP, Pm4::R_WAIT_MEM_32);
	...
	cmd[0] = PS5SIM_PM4(9, Pm4::IT_NOP, Pm4::R_WAIT_MEM_64);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbPushMarker(CommandBuffer* buf, const char* str,
                                              uint32_t /*color*/) {
	if (buf == nullptr) {
		return nullptr;
	}
	...
	cmd[0] = PS5SIM_PM4(size, Pm4::IT_NOP, Pm4::R_PUSH_MARKER);
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbPopMarker(CommandBuffer* buf) {
	...
	cmd[0] = PS5SIM_PM4(2, Pm4::IT_NOP, Pm4::R_POP_MARKER);
	cmd[1] = 0;
...
uint32_t* PS5SIM_SYSV_ABI GraphicsDcbSetFlip(CommandBuffer* buf, uint32_t video_out_handle,
	...
	cmd[0] = PS5SIM_PM4(6, Pm4::IT_NOP, Pm4::R_FLIP);
	cmd[1] = video_out_handle;
	cmd[2] = display_buffer_index;
	cmd[3] = flip_mode;
...
static bool dcb_has_queued_interrupt(const uint32_t* dcb, uint32_t size_in_dwords) {
	...
	for (uint32_t offset = 0; offset < size_in_dwords;) {
		...
		auto op = (cmd_id >> 8u) & 0xffu;
		if (op == Pm4::IT_NOP && PS5SIM_PM4_R(cmd_id) == Pm4::R_RELEASE_MEM && len >= 6) {
...
static std::vector<uint64_t> collect_acb_wait_addresses(const uint32_t* acb,
	...
	for (uint32_t offset = 0; offset < size_in_dwords;) {
		...
		auto op = (cmd_id >> 8u) & 0xffu;
		if (op == Pm4::IT_NOP && PS5SIM_PM4_R(cmd_id) == Pm4::R_WAIT_MEM_32 && len >= 7) {
			auto address = static_cast<uint64_t>(acb[offset + 1]) |
			               (static_cast<uint64_t>(acb[offset + 2]) << 32u);
			if (address != 0) {
				addresses.push_back(address);
			}
		} else if (op == Pm4::IT_NOP && PS5SIM_PM4_R(cmd_id) == Pm4::R_WAIT_MEM_64 && len >= 9) {
			auto address = static_cast<uint64_t>(acb[offset + 1]) |
			               (static_cast<uint64_t>(acb[offset + 2]) << 32u);
			if (address != 0) {
				addresses.push_back(address);
		...
		if (!wait_addresses.empty() && Gen5::g_pending_graphics_segment.start != nullptr) {
			...
			while (scan < Gen5::g_pending_graphics_segment.end &&
				...
				if (((cmd_id >> 8u) & 0xffu) == Pm4::IT_NOP &&
				    PS5SIM_PM4_R(cmd_id) == Pm4::R_RELEASE_MEM && len >= 7) {
					auto release_addr =
