...
// NOLINTNEXTLINE(readability-function-cognitive-complexity)
static int ps5sim_printf_internal(bool sn, char* sn_s, size_t sn_n, const char* format,
                                VaList* va_list) {
...
static int ps5sim_vprintf(const char* format, VaList* va_list) {
	return ps5sim_printf_internal(false, nullptr, 0, format, va_list);
}

static int ps5sim_printf_ctx(VaContext* ctx) {
	const char* format = VaArg_ptr<const char>(&ctx->va_list);

	return ps5sim_printf_internal(false, nullptr, 0, format, &ctx->va_list);
}

static int ps5sim_snprintf_ctx(VaContext* ctx) {
	char*       s      = VaArg_ptr<char>(&ctx->va_list);
	size_t      n      = VaArg_size_t(&ctx->va_list);
	const char* format = VaArg_ptr<const char>(&ctx->va_list);

	return ps5sim_printf_internal(true, s, n, format, &ctx->va_list);
}

static int PS5SIM_SYSV_ABI ps5sim_printf_std(VA_ARGS) {
	VA_CONTEXT(ctx); // NOLINT(cppcoreguidelines-pro-type-member-init,hicpp-member-init)

	return ps5sim_printf_ctx(&ctx);
}

guest_printf_std_func_t GetGuestPrintfStdFunc() {
	return reinterpret_cast<guest_printf_std_func_t>(ps5sim_printf_std);
}

guest_printf_ctx_func_t GetGuestPrintfCtxFunc() {
	return ps5sim_printf_ctx;
}

guest_snprintf_ctx_func_t GetGuestSnprintfCtxFunc() {
	return ps5sim_snprintf_ctx;
}

guest_vprintf_func_t GetGuestVprintfFunc() {
	return ps5sim_vprintf;
}
