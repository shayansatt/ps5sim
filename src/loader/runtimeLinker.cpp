...
using entry_func_t  = PS5SIM_SYSV_ABI void (*)(EntryParams* params, atexit_func_t atexit_func);
using module_ini_fini_func_t = PS5SIM_SYSV_ABI int (*)(size_t args, const void* argp,
                                                     module_func_t func);
...
static PS5SIM_SYSV_ABI uint64_t ResolveImportStubWithId(uint64_t record_id);
...
static PS5SIM_SYSV_ABI void RunEntry(uint64_t addr, EntryParams* params, atexit_func_t atexit_func,
                                   void* stack_top) {
...
static void PS5SIM_SYSV_ABI StackwalkX86(uint64_t rbp, void** stack, int* depth, uintptr_t stack_addr,
...
static void PS5SIM_SYSV_ABI SysStackWalkX86(uint64_t rbp, uint64_t rsp, void** stack, int* depth) {
	if (rsp == 0 || rbp < rsp) {
		*depth = 0;
		return;
	}
...
void PS5SIM_SYSV_ABI SysStackWalkX86(uint64_t rbp, void** stack, int* depth) {
	SysStackWalkX86(rbp, rbp, stack, depth);
}

static bool Ps5SimExceptionHandler(const Common::HostException::ExceptionInfo& exception_info) {
	...
	LOGF("ps5sim_exception_handler: %016" PRIx64 "\n", info->exception_address);
#if PS5SIM_PLATFORM == PS5SIM_PLATFORM_WINDOWS
...
static void PS5SIM_SYSV_ABI ProgramExitHandler() {
...
static RelocationInfo GetRelocationInfo(Elf64_Rela* r, Program* program) {
	...
	// PS5SIM_PROFILER_BLOCK("1");

	RelocationInfo ret;
	// SymbolRecord   sr {};

	// PS5SIM_PROFILER_END_BLOCK;

	// PS5SIM_PROFILER_BLOCK("2");
...
__attribute__((naked)) static PS5SIM_SYSV_ABI void RelocateHandlerReturnStub() {
	asm volatile("addq $8, %rsp\n\t"
	             "retq\n");
}

static PS5SIM_SYSV_ABI uint64_t RelocateHandler(RelocateHandlerStack s) {
	auto*       stack     = s.stack;
...
static PS5SIM_MS_ABI uint8_t* TlsMainGetAddr() {
...
void RuntimeLinker::Execute() {
	PS5SIM_PROFILER_THREAD("Thread_Main");
...
void RuntimeLinker::LoadProgramToMemory(Program* program) {
	...
	if (!Common::HostException::InstallHandler(Ps5SimExceptionHandler)) {
